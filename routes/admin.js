...
router.post('/users/:id/add-balance', async (req, res) => {
  try {
    ...
    await user.save({ validateBeforeSave: false });
    await Transaction.create({ user: user._id, type: 'deposit', amount: parseFloat(amount), status: 'approved', paymentMethod: 'system', adminNote: note || 'অ্যাডমিন কর্তৃক যোগ করা হয়েছে', processedBy: req.user._id, processedAt: new Date() });
